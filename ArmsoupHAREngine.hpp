...
namespace HEngine {
	...
	class Filesystem {
	public:
		static inline bool fileExists(std::string_view path) {
			return fs::exists(path) && fs::is_regular_file(path);
		}

		static inline bool dirExists(std::string_view path) {
			return fs::exists(path) && fs::is_directory(path);
		}

		static inline void writeFile(const std::string& path, std::string_view content) {
			std::ofstream f(path);
			if (f.is_open()) {
				f << content;
				f.close();
		...
		static inline void writeLog(std::string_view message) {
		...
		static inline void Achievements(std::string_view message) {
	...
	public:
		...
		static inline void ErrorGame(std::string_view message, int action) { // 1 - exit, 2 - abort, 3 – nothing (the developer will choose the action)
		...
		static inline bool isKeyPressed(int vKey) {
			return (GetAsyncKeyState(vKey) & 0x8000) != 0;
		}

		static inline void print_slow(std::string_view text, int delay) { // Recommended delay - 3
			timeBeginPeriod(1);
			for (char c : text) {
		...
		static inline void SetConsoleWidthHeight(int newWidth, short newHeight) {
		...
		static inline void setColor(const std::string& colorCode) {
			HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
			try {
	...
	public:
		...
		static inline void loadGame() {
			...
			if (Success == 1) Filesystem::writeLog("Successfully loaded");
		}
	};
	// 3D
	class Vector3D {
	public:
		static inline void render3D_Ultimate(float playerAngle, const std::string& mapData, int mapWidth, int mapHeight) {
			...
			for (int x = 0; x < screenWidth; x++) {
				...
				for (float d = 0.2f; d < MAX_DIST; d += 0.05f) {
					...
					if (mapData[checkY * mapWidth + checkX] != '.' && mapData[checkY * mapWidth + checkX] != '\0') {
						distanceToWall = d;
